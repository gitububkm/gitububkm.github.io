...
```plaintext
/
├── index.html        # Главная страница (оверлей-согласие)
...
│   ├── script.js     # Скрипты для сбора данных и оверлей-согласия
│   ├── style.css     # Дополнительные стили
│   └── router.js     # Роутинг для главной страницы
├── backend/          # Backend код
│   └── server.py     # Flask сервер для сбора данных, Telegram
...
### Функционал
Сайт собирает информацию о посетителях (браузер, ОС, железо, геолокация) и отправляет данные администратору в Telegram. Сбор запускается ТОЛЬКО после согласия пользователя через модальное оверлей‑уведомление на главной странице.
...
- Оверлей‑уведомление с двумя кнопками: «Согласен» (сбор сразу), «Отказаться» (редирект на google.com)
- Сбор данных через JavaScript API, отправка на бэкенд
- Исключения/дубликаты учитываются на сервере (см. ниже)

**Backend (Render.com):**
- Flask‑сервер принимает данные
- Обогащение данными геолокации через ipinfo.io
- Отправка данных в Telegram бота (документ .txt + inline‑кнопка удаления)
- Реестр исключений: точные дубликаты по хэшу не отправляются; «похожий» контент (≥50% схожести с последними 50 сообщениями) не отправляется

**Telegram Bot:**
- Принимает `.txt` файлы с собранными данными
- Кнопка "🗑️ Удалить это сообщение" под каждым файлом (удаление доступно по callback)

### Структура Backend
```
backend/
├── server.py          # Flask приложение
│   ├── /collect       # Прием данных от фронтенда (порог уникальности ≥50% новизны)
...
- Уведомление и явное согласие через модальное окно на главной
...
### Лимиты и защита
- Rate limiting: при превышении лимитов запросов IP блокируется на 90 секунд
- Блок автоматизированных инструментов (curl/wget/requests и т.п.)
